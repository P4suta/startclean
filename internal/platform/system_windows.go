...
func knownFolder(id windows.GUID) (string, error) {
	...
	result, _, _ := procSHGetKnownFolderPath.Call(
		uintptr(unsafe.Pointer(&id)), 0, 0, uintptr(unsafe.Pointer(&raw)),
	)
	...
	defer func() { _, _, _ = procCoTaskMemFree.Call(uintptr(unsafe.Pointer(raw))) }()
...
func (s *System) Target(linkPath string) (string, error) {
	...
	result = comCall(shellLink, 0, uintptr(unsafe.Pointer(&iidPersistFile)), uintptr(unsafe.Pointer(&persistFile)))
	if failedHRESULT(result) {
	...
	result = comCall(persistFile, 5, uintptr(unsafe.Pointer(linkPathUTF16)), 0)
	if failedHRESULT(result) {
	...
	result = comCall(
		shellLink,
		3,
		uintptr(unsafe.Pointer(&targetBuffer[0])),
...
func (s *System) ExpandEnvironment(value string) (string, error) {
	...
	required, _, callErr := procExpandEnvW.Call(uintptr(unsafe.Pointer(source)), 0, 0)
	if required == 0 {
...
func (s *System) DriveKind(root string) (core.DriveKind, error) {
	...
	kind, _, _ := procGetDriveTypeW.Call(uintptr(unsafe.Pointer(rootUTF16)))
	switch kind {
	case driveFixed:
...
func notifyShell(event uintptr, path string) {
	...
	_, _, _ = procSHChangeNotify.Call(event, shcnfPathW, uintptr(unsafe.Pointer(encoded)), 0)
